...
test("Testing One Story, StoryData", function (): void {
    ...
    expect($storyblokData->get("name"))
        ...
        ->toBe("2024-02-08")
        ->and($storyblokData->tagListAsArray())
        ->toBe(["tag1", "tag2"])
        ->and($storyblokData->tagListAsString())
        ->toBe("tag1, tag2")
...
test("Testing Creating story with error", function (): void {
    $responses = [\mockResponse("empty-story", 401)];
...
test("Create story encodes data correctly as JSON", function (): void {
    ...
    $callback = function ($method, $url, array $options) use (
        $expectedStoryData,
    ): void {
...
test("Testing setting Story methods, StoryData", function (): void {
    $responses = [
        \mockResponse("one-story", 200),
        \mockResponse("one-story", 200),
        \mockResponse("empty-story", 404),
    ...
    expect($storyblokData->get("name"))
        ...
        ->and($storyblokData->createdAt())
        ->toBe("2024-02-08")
        ->and($storyblokData->getInt("parent_id"))
        ->toBe(123456)
        ->and($storyblokData->folderId())
        ->toBe(123456);
