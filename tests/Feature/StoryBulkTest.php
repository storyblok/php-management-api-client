...
test("StoryApi works with custom logger", function (): void {
    ...
    expect($mockLogger->logs)
        ->not->toBeEmpty()
        ->and($mockLogger->logs[0]["level"])
        ->toBe("error")
        ->and($mockLogger->logs[0]["message"])
        ->toBe("API error");
});
...
test("Testing list of stories max retry on the first page", function (): void {
    $responses = [
...
test("Testing all() with some sparse 429", function (): void {
    $responses = [
        \mockResponse("list-stories-page-1", 429),
        \mockResponse("list-stories-page-1", 200, [
            "total" => 6,
    ...
    foreach (
        ...
        expect($story->name())->toBe($expectedNames[$i]);
        ++$i;
    }
...
test("Testing all() with a lot of 429 on second page", function (): void {
    $responses = [
        \mockResponse("list-stories-page-1", 429),
        \mockResponse("list-stories-page-1", 200, [
            "total" => 6,
...
test("Testing 429 on first page with no response headers", function (): void {
    $responses = [
        \mockResponse("list-stories-page-1", 429),
        \mockResponse("list-stories-page-1", 200, [
            "total" => 6,
...
test(
    "createBulk handles rate limiting and creates multiple stories",
    function (): void {
        $mockLogger = new class extends NullLogger {
            public array $logs = [];
...
test(
    "createBulk throws exception when max retries is reached",
    function (): void {
        $mockLogger = new class extends NullLogger {
            public array $logs = [];
        ...
        expect(
            fn(): array => iterator_to_array(
                $storyApi->createStories($stories),
            ),
        )->toThrow(
            \Storyblok\ManagementApi\Exceptions\StoryblokApiException::class,
            "Rate limit exceeded maximum retries",
        ...
        expect($warningCount)->toBe(3)->and($hasErrorLog)->toBeTrue();
        ...
        expect($lastErrorLog["context"])
            ->toHaveKey("story_name")
            ->and($lastErrorLog["context"]["story_name"])
            ->toBe("Story 1");
    },
);
