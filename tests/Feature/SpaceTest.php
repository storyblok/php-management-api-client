...
test("Testing making space", function (): void {
    ...
    expect($spaceData)->toBeInstanceOf(
        \Storyblok\ManagementApi\Data\Space::class,
    );
    $spacesData = \Storyblok\ManagementApi\Data\Spaces::make([]);
    expect($spacesData)->toBeInstanceOf(
        \Storyblok\ManagementApi\Data\Spaces::class,
    );
    $responses = [
...
test("Testing One space, Space", function (): void {
    ...
    expect($storyblokData->get("name"))
        ->toBe("Example Space")
        ->and($storyblokData->name())
        ->toBe("Example Space")
        ->and($storyblokData->ownerId())
        ->toBe("1114")
        ->and($storyblokData->id())
        ->toBe("680")
        ->and($storyblokData->createdAt())
        ->toBe("2018-11-10")
        ->and($storyblokData->updatedAt())
        ->toBe("2018-11-11")
        ->and($storyblokData->planDescription())
        ->toBe("Starter (Trial)")
        ->and($storyblokData->domain())
        ->toBe("https://example.storyblok.com")
        ->and($storyblokData->firstToken())
        ->toBe("8IE7MzYCzw5d7KLckDa38Att")
        ->and($storyblokData->isDemo())
        ->toBeFalse()
...
test("Testing Creating One space, Space", function (): void {
    ...
    expect($spaceData->get("name"))
        ->toBe("New Name")
        ->and($spaceData->createdAt())
        ->toBe("")
        ->and($spaceData->get("domain"))
        ->toBe("https://example.storyblok.com");
...
test("throws exception in creating space", function (): void {
    $responses = [\mockResponse("empty-space", 404)];

    $client = new MockHttpClient($responses);
    $mapiClient = ManagementApiClient::initTest($client);
    $spaceApi = new SpaceApi($mapiClient);
    $storyblokResponse = $spaceApi->create(new Space(""));
})->throws(
